    ...
    private void protocolSwitch(String[] command) {
        ...
        if (protocol.getNumArgs() == command.length - 1) {
            switch (protocol) {
                ...
                case BROADCAST -> this.receiveMessage(Arrays.copyOfRange(command, 1, command.length), "Public");
                case WHISPER -> this.receiveMessage(Arrays.copyOfRange(command, 1, command.length), "Private");
